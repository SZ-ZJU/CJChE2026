...
#
# # ==== 14. 保存结果 ====
# out_path = "volume_actual_vs_pred_with_residual_correction_train_test_split.xlsx"
#
...
id_col = df.columns[0]
group_cols = df.columns[13:32]   # 第14~32列：19个基团
temp_cols = df.columns[32:42]    # 第33~42列：温度
v_cols = df.columns[42:52]       # 第43~52列：体积
...
# ============================================================
# 3. 体积参考点子模型：Volume at boiling temperature
#    保持原逻辑：全数据训练
# ============================================================
df_Tb = pd.read_excel("selected_25_descriptors_boiling.xlsx")

target_volume_tb = "volume at boiling temperature"

X_Tb = df_Tb.drop(columns=[target_volume_tb]).copy()
y_Tb = pd.to_numeric(df_Tb[target_volume_tb], errors="coerce").values

for col in X_Tb.columns:
    X_Tb[col] = pd.to_numeric(X_Tb[col], errors="coerce")

valid_tb_desc_mask = (
    np.isfinite(X_Tb.values).all(axis=1)
    & np.isfinite(y_Tb)
)

# 如果描述符文件行数与主表不一致，这里仍按原逻辑要求一致
if len(X_Tb) != len(df):
    raise ValueError(
        f"selected_25_descriptors_boiling.xlsx 行数 = {len(X_Tb)}，"
        f"与 volume208.xlsx 有效物质数 = {len(df)} 不一致。"
        "当前代码默认两个文件逐行对应。"
    )
...
rf_Tb.fit(
    X_Tb.loc[valid_tb_desc_mask],
    y_Tb[valid_tb_desc_mask]
)

Volume_Tb_all = rf_Tb.predict(X_Tb)


# ============================================================
# 4. Tb 子模型：保持原逻辑，全数据训练
# ============================================================
Tb_raw = pd.to_numeric(df.iloc[:, 5], errors="coerce").values
Tb0 = 222.543
...
Volume_Tb_train = Volume_Tb_all[train_row_mask]
Volume_Tb_test = Volume_Tb_all[test_row_mask]
...
for i in range(len(train_df)):
    for tcol, vcol in zip(temp_cols, v_cols):
        ...
        if (
            ...
            or not np.isfinite(Volume_Tb_train[i])
            or not np.isfinite(G_train[i]).all()
        ):
            continue

        Tb_i = Tb_pred_train[i]
        Volume_Tb_i = Volume_Tb_train[i]
...
# 7. 生成基准体积预测
# ============================================================
def build_baseline_predictions(df_part, G_part, Tb_pred_part, Volume_Tb_part):
    pred_df = pd.DataFrame(index=df_part.index, columns=v_cols, dtype=float)

    for i in range(len(df_part)):
        if (
            not np.isfinite(Tb_pred_part[i])
            or not np.isfinite(Volume_Tb_part[i])
        ...
        for tcol, vcol in zip(temp_cols, v_cols):
            ...
            Xj = (Tj - Tb_i) * G_part[i]
            pred_df.at[i, vcol] = Volume_Tb_i + Xj @ A_vec
...
def build_residual_dataset(df_part, G_part, Tb_pred_part, Volume_Tb_part, baseline_pred_df):
    residual_features = []
    residual_targets = []
    ...
    return np.array(residual_features, dtype=float), np.array(residual_targets, dtype=float)
...
# ============================================================
def build_final_predictions(df_part, G_part, Tb_pred_part, Volume_Tb_part, baseline_pred_df):
    pred_df = pd.DataFrame(index=df_part.index, columns=v_cols, dtype=float)
...
# 残差模型在 residual 数据集上的直接预测，用于 residual 层面评估
residual_train_pred = residual_model.predict(residual_X_train_scaled)
residual_test_pred = residual_model.predict(residual_X_test_scaled)
...
print("\nFinal_model 完整数据集 Volume 预测偏差 1%，5%，10%分别为：")
...
for tcol, vcol in zip(temp_cols, v_cols):
    m = (
        (~train_df[tcol].isna())
        & (~train_df[vcol].isna())
        & (~V_pred_final_train[vcol].isna())
    )

    if m.any():
        v_true = train_df.loc[m, vcol].to_numpy(dtype=float)
        v_pred = V_pred_final_train.loc[m, vcol].to_numpy(dtype=float)
...
for tcol, vcol in zip(temp_cols, v_cols):
    m = (
        (~test_df[tcol].isna())
        & (~test_df[vcol].isna())
        & (~V_pred_final_test[vcol].isna())
    )

    if m.any():
        v_true = test_df.loc[m, vcol].to_numpy(dtype=float)
        v_pred = V_pred_final_test.loc[m, vcol].to_numpy(dtype=float)
    ...
    for idx in range(len(df_part)):
        ...
        for j, (tcol, vcol) in enumerate(zip(temp_cols, v_cols), start=1):
            ...
            rows.append({
                ...
                "residual_correction": residual_correction,
                "T_ref": Tb_pred_part[idx],
                "Volume_ref": Volume_ref_part[idx]
            })
...
print(f"R²  = {final_metrics_all['R2']:.4f}")
...
# 16. 输出模型结构记录
# ============================================================
print("\n当前 Volume baseline + GBDT residual 模型结构:")
print("Volume_Tb_submodel: RandomForestRegressor(random_state=42), input = selected_25_descriptors_boiling.xlsx")
print("Tb_submodel: HuberRegressor(max_iter=10000), input = PolynomialFeatures(Nk, degree=2)")
print("Baseline: Volume_baseline = Volume_Tb_pred + (T - Tb_pred) * sum(Ak * Nk)")
...
print("Residual target: Volume_actual - Volume_baseline")
print("Residual features: Nk + T + (T-Tb) + T/Tb + ln(T) + Volume_baseline + Tb_pred + Volume_Tb_pred")
print("Final prediction: Volume_final = Volume_baseline + residual_pred")
