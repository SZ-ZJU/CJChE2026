...
# # 1. 读取数据
# # =========================
# file_path = "Transformed_Gibbs_Dataset.xlsx"
# sheet_name = "Sheet2"
# df = pd.read_excel("Transformed_Gibbs_Dataset.xlsx", sheet_name=sheet_name).copy()
# target_col = "Gibbs"
...
#
# output_file = "prediction_vs_actual_enthalpy_train_test_split_by_material.xlsx"
...
if material_col is None:
    ...
    if len(df) % rows_per_material != 0:
        print(
            f"警告：总行数 {len(df)} 不是 {rows_per_material} 的整数倍，"
            f"最后一个物质组可能不完整。"
        )
...
train_df = df[
    df[material_col].isin(train_materials)
].copy()

test_df = df[
    df[material_col].isin(test_materials)
].copy()
...
print("\nTransformed Gibbs RF 完整数据集预测偏差 1%，5%，10%分别为：")
...
# 12. 保存 Excel
# =========================
output_file = "prediction_vs_actual_Gibbs_train_test_split_by_material.xlsx"
...
feature_importance_file = "Transformed_Gibbs_RF_feature_importance.xlsx"
...
# 14. 输出模型结构记录
# =========================
print("\n当前 Transformed Gibbs RF 直接预测模型结构:")
print("Dataset: Transformed_Gibbs_Dataset.xlsx")
print("Sheet: Sheet2")
print("Target: Gibbs")
