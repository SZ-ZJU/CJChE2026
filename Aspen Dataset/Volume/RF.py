...
#     "Volume_measured (m³/mol)": y_train,
#     "Volume_predicted (m³/mol)": y_train_pred,
...
#     "Volume_measured (m³/mol)": y_test,
#     "Volume_predicted (m³/mol)": y_test_pred,
...
#
# output_file = "Volume_RF_train_test_split_by_material.xlsx"   # 修改输出文件名以区分
...
v_cols = df.columns[42:52]        # 10个体积列
...
def build_point_dataset(df_part):
    ...
    for _, row in df_part.iterrows():
        ...
        vals = pd.to_numeric(row[v_cols], errors="coerce").values.astype(float)
...
print("\nVolume RF 完整数据集预测偏差 1%，5%，10%分别为：")
...
train_result = pd.DataFrame({
    ...
    "Material_ID": material_ids_train,
    "Temperature (K)": temperatures_train,
    "Volume_measured": y_train,
    "Volume_predicted": y_train_pred,
    "Absolute Error": np.abs(y_train - y_train_pred),
    "Relative Error (%)": rel_err_train
})
...
test_result = pd.DataFrame({
    ...
    "Material_ID": material_ids_test,
    "Temperature (K)": temperatures_test,
    "Volume_measured": y_test,
    "Volume_predicted": y_test_pred,
    "Absolute Error": np.abs(y_test - y_test_pred),
    "Relative Error (%)": rel_err_test
})
...
all_result = pd.DataFrame({
    ...
    "Volume_measured": y_all_true,
    "Volume_predicted": y_all_pred,
...
output_file = "Volume_RF_train_test_split_by_material.xlsx"
...
feature_importance_file = "Volume_RF_feature_importance.xlsx"
...
# 16. 输出模型结构记录
# ============================================================
print("\n当前 Volume RF 直接预测模型结构:")
print("Dataset: volume208.xlsx")
print("Target: ordinary Volume")
