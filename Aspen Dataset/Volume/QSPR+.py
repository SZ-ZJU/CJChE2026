...
# normal_target = "volume at normal temperature"
#
# boiling_desc_file = "selected_25_descriptors_boiling.xlsx"
# boiling_target = "volume at boiling temperature"
#
# transformed_file = "Transformed_volume_Dataset.xlsx"
# final_target_col = "Volume"
#
...
#     "volume_normal_pred": HVap_298_all,
#     "volume_boiling_pred": HVap_Tb_all,
#     "Tb_pred": Tb_pred_all,
#     "slope": slope_all
# })
...
#
# transformed_with_slope_path = "Transformed_volume_with_slope.xlsx"
...
# train_result["Predicted_Volume"] = y_train_pred
...
# test_result["Predicted_Volume"] = y_test_pred
...
#
# final_output = "prediction_vs_actual_Volume_with_slope_train_test_split.xlsx"
...
# ============================================================
# 2. 训练 normal temperature volume 子模型：全数据
# ============================================================
df_298 = pd.read_excel(normal_desc_file).copy()
...
volume_normal_pred_all = rf_298.predict(X_298_full)


# ============================================================
# 3. 训练 boiling temperature volume 子模型：全数据
# ============================================================
df_Tb = pd.read_excel(boiling_desc_file).copy()
...
volume_boiling_pred_all = rf_Tb.predict(X_Tb_full)
...
if len(volume_normal_pred_all) != n_main:
    raise ValueError(
        f"{normal_desc_file} 预测行数 = {len(volume_normal_pred_all)}，"
        f"但主表物质数 = {n_main}，无法按顺序一一对应。"
    )

if len(volume_boiling_pred_all) != n_main:
    raise ValueError(
        f"{boiling_desc_file} 预测行数 = {len(volume_boiling_pred_all)}，"
        f"但主表物质数 = {n_main}，无法按顺序一一对应。"
    )
...
slope_all = (
    volume_boiling_pred_all
    - volume_normal_pred_all
) / safe_denom

slope_df = pd.DataFrame({
    "Material_ID": material_ids,
    "volume_normal_pred": volume_normal_pred_all,
    "volume_boiling_pred": volume_boiling_pred_all,
    "Tb_pred": Tb_pred_all,
    "slope": slope_all
})
...
trans_df["volume_normal_pred"] = np.repeat(
    slope_df["volume_normal_pred"].values,
    rows_per_material
)

trans_df["volume_boiling_pred"] = np.repeat(
    slope_df["volume_boiling_pred"].values,
    rows_per_material
)
...
print("\nTransformed Volume + slope RF 完整数据集预测偏差 1%，5%，10%分别为：")
...
all_result["Predicted_Volume"] = y_all_pred
...
feature_importance_file = "Transformed_Volume_with_slope_RF_feature_importance.xlsx"
...
# 16. 输出模型结构记录
# ============================================================
print("\n当前 Transformed Volume + slope + RF 模型结构:")
print("volume_normal_submodel: RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=-1)")
print("volume_boiling_submodel: RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=-1)")
print("Tb_submodel: HuberRegressor(max_iter=10000), input = PolynomialFeatures(Nk, degree=2)")
print("slope = (volume_boiling_pred - volume_normal_pred) / (Tb_pred - 298.15)")
print("Final target: Volume")
print("Final model: RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=-1)")
print("Final input features: transformed numeric features + slope + volume_normal_pred + volume_boiling_pred + Tb_pred")
