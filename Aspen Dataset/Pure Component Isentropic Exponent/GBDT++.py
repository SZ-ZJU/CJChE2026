...
# y_Tb = pd.to_numeric(
#     df_Tb["ASPEN isentropic exponent at boiling Temperature(bar)"],
#     errors="coerce"
# ).values
#
...
#
# # ==== 10. 生成最终预测（基准 + 残差修正），分别对 train / test ====
...
#
# # ==== 14. 保存结果 ====
# out_path = "isentropic_exponent_actual_vs_pred_with_residual_correction_train_test_split.xlsx"
#
...
#                 "Isentropic_Exponent_ref": HVap_ref_part[idx]
#             })
...
# ============================================================
df = pd.read_excel(
    "pure component isentropic exponent 207.xlsx",
    sheet_name="Sheet1"
)
...
IE_Tb_all = rf_Tb.predict(X_Tb_predict)

if len(IE_Tb_all) != len(df):
    raise ValueError(
        f"selected_25_descriptors_boiling.xlsx 预测行数 = {len(IE_Tb_all)}，"
        f"与主表物质数 = {len(df)} 不一致。"
    )
...
IE_Tb_train = IE_Tb_all[train_row_mask]
IE_Tb_test = IE_Tb_all[test_row_mask]
...
for i in range(len(train_df)):
    for tcol, vcol in zip(temp_cols, v_cols):
        Tj = train_df.at[i, tcol]
        IEj = train_df.at[i, vcol]

        if np.isnan(Tj) or np.isnan(IEj):
            continue

        if (
            not np.isfinite(Tb_pred_train[i])
            or not np.isfinite(IE_Tb_train[i])
            or not np.isfinite(G_train[i]).all()
        ):
            continue

        Tb_i = Tb_pred_train[i]
        IE_Tb_i = IE_Tb_train[i]
...
# ============================================================
# 7. 生成 baseline 等熵指数预测
# ============================================================
def build_baseline_predictions(df_part, G_part, Tb_pred_part, IE_Tb_part):
    ...
    for i in range(len(df_part)):
        if (
            ...
            or not np.isfinite(IE_Tb_part[i])
        ...
        for tcol, vcol in zip(temp_cols, v_cols):
            ...
            Xj = (Tj - Tb_i) * G_part[i]
            pred_df.at[i, vcol] = IE_Tb_i + Xj @ A_vec

    return pred_df


IE_pred_baseline_train = build_baseline_predictions(
    train_df,
    G_train,
    Tb_pred_train,
    IE_Tb_train
...
IE_pred_baseline_test = build_baseline_predictions(
    test_df,
    G_test,
    Tb_pred_test,
    IE_Tb_test
...
def build_residual_dataset(df_part, G_part, Tb_pred_part, IE_Tb_part, baseline_pred_df):
    residual_features = []
    residual_targets = []

    for tcol, vcol in zip(temp_cols, v_cols):
        Tj = df_part[tcol].to_numpy(dtype=float)
        IEj = df_part[vcol].to_numpy(dtype=float)
        ...
        for i in np.where(msk)[0]:
            ...
            residual_features.append(all_features)
            residual_targets.append(IEj[i] - baseline_pred)
...
# ============================================================
def build_final_predictions(df_part, G_part, Tb_pred_part, IE_Tb_part, baseline_pred_df):
    pred_df = pd.DataFrame(
        index=df_part.index,
        columns=v_cols,
        dtype=float
...
IE_pred_final_train = build_final_predictions(
    train_df,
    G_train,
    Tb_pred_train,
    IE_Tb_train,
...
IE_pred_final_test = build_final_predictions(
    test_df,
    G_test,
    Tb_pred_test,
    IE_Tb_test,
...
print("\nFinal_model 完整数据集 Isentropic Exponent 预测偏差 1%，5%，10%分别为：")
...
for tcol, vcol in zip(temp_cols, v_cols):
    m = (
        (~train_df[tcol].isna())
        & (~train_df[vcol].isna())
        & (~IE_pred_final_train[vcol].isna())
    )

    if m.any():
        v_true = train_df.loc[m, vcol].to_numpy(dtype=float)
        v_pred = IE_pred_final_train.loc[m, vcol].to_numpy(dtype=float)
...
for tcol, vcol in zip(temp_cols, v_cols):
    m = (
        (~test_df[tcol].isna())
        & (~test_df[vcol].isna())
        & (~IE_pred_final_test[vcol].isna())
    )

    if m.any():
        v_true = test_df.loc[m, vcol].to_numpy(dtype=float)
        v_pred = IE_pred_final_test.loc[m, vcol].to_numpy(dtype=float)
...
def build_long_compare(
    ...
    for idx in range(len(df_part)):
        ...
        for j, (tcol, vcol) in enumerate(zip(temp_cols, v_cols), start=1):
            ...
            err_base = (
                IE_base - IE_actual
                if pd.notna(IE_base) and pd.notna(IE_actual)
                else np.nan
            )

            err_final = (
                IE_final - IE_actual
                if pd.notna(IE_final) and pd.notna(IE_actual)
                else np.nan
            )

            residual_correction = (
                IE_final - IE_base
                if pd.notna(IE_final) and pd.notna(IE_base)
                else np.nan
            )

            rel_err_base = (
                abs((IE_base - IE_actual) / IE_actual) * 100
                if pd.notna(IE_base) and pd.notna(IE_actual) and abs(IE_actual) > 1e-12
                else np.nan
            )

            rel_err_final = (
                abs((IE_final - IE_actual) / IE_actual) * 100
                if pd.notna(IE_final) and pd.notna(IE_actual) and abs(IE_actual) > 1e-12
                else np.nan
            )
            ...
            rows.append({
                ...
                "temp_col": tcol,
                "T": T,
                "Isentropic_Exponent_actual": IE_actual,
                "Isentropic_Exponent_baseline": IE_base,
                "Isentropic_Exponent_final": IE_final,
                "error_baseline": err_base,
                "error_final": err_final,
                ...
                "residual_correction": residual_correction,
                "T_ref": Tb_pred_part[idx],
                "Isentropic_Exponent_ref": IE_ref_part[idx]
            })
...
# 16. 输出模型结构记录
# ============================================================
print("\n当前 Isentropic Exponent baseline + GBDT residual 模型结构:")
print("Isentropic_Tb_submodel: RandomForestRegressor(random_state=42, n_jobs=-1), input = selected_25_descriptors_boiling.xlsx")
print("Tb_submodel: HuberRegressor(max_iter=10000), input = PolynomialFeatures(Nk, degree=2)")
print("Baseline: IE_baseline = IE_Tb_pred + (T - Tb_pred) * sum(Ak * Nk)")
...
print("Residual target: IE_actual - IE_baseline")
print("Residual features: Nk + T + (T-Tb) + T/Tb + ln(T) + IE_baseline + Tb_pred + IE_Tb_pred")
print("Final prediction: IE_final = IE_baseline + residual_pred")
