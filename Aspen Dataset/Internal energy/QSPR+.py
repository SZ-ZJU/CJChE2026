...
# main_sheet = "Sheet1"
#
# file_298 = "selected_25_descriptors_normal.xlsx"
# target_298 = "internal energy at normal temperature"
#
# file_tb = "selected_25_descriptors_boiling.xlsx"
# target_tb = "internal energy at boiling temperature"
#
# transformed_file = "Transformed_internal_energy_Dataset.xlsx"
# final_target_col = "Internal energy"
#
...
#
# # 仅保留数值列
# X_298 = X_298.select_dtypes(include=[np.number]).copy()
...
#
# mask_tb_model = np.isfinite(Tb_raw) & np.isfinite(Nk_poly).all(axis=1)
#
# model_Tb = HuberRegressor(max_iter=10000)
# model_Tb.fit(
#     Nk_poly[mask_tb_model],
#     np.exp(Tb_raw[mask_tb_model] / Tb0)
# )
#
...
#     raise ValueError(
#         f"{file_298} 预测行数 = {len(HVap_298_all)}，但主表物质数 = {n_main}，无法按顺序一一对应。"
#     )
# if len(HVap_Tb_all) != n_main:
#     raise ValueError(
#         f"{file_tb} 预测行数 = {len(HVap_Tb_all)}，但主表物质数 = {n_main}，无法按顺序一一对应。"
#     )
#
# # =========================
# # 6. 计算 slope
...
#
# transformed_with_slope_path = "Transformed_internal_energy_with_slope.xlsx"
...
# train_result["Predicted_Internal_Energy"] = y_train_pred
...
# test_result["Predicted_Internal_Energy"] = y_test_pred
...
#
# final_output = "prediction_vs_actual_Internal_energy_with_slope_train_test_split.xlsx"
...
# 如果 X_298 中仍有缺失，用训练有效集的中位数填充，避免 predict 报错
X_298_predict = X_298.copy()
X_298_predict = X_298_predict.fillna(X_298_valid.median(numeric_only=True))

Internal_energy_298_all = rf_298.predict(X_298_predict)


# =========================
# 3. 训练 Tb 内能子模型，并预测全部物质
# =========================
df_Tb = pd.read_excel(file_tb).copy()
...
# 如果 X_Tb 中仍有缺失，用训练有效集的中位数填充，避免 predict 报错
X_Tb_predict = X_Tb.copy()
X_Tb_predict = X_Tb_predict.fillna(X_Tb_valid.median(numeric_only=True))

Internal_energy_Tb_all = rf_Tb.predict(X_Tb_predict)
...
if len(Internal_energy_298_all) != n_main:
    raise ValueError(
        f"{file_298} 预测行数 = {len(Internal_energy_298_all)}，"
        f"但主表物质数 = {n_main}，无法按顺序一一对应。"
    )

if len(Internal_energy_Tb_all) != n_main:
    raise ValueError(
        f"{file_tb} 预测行数 = {len(Internal_energy_Tb_all)}，"
        f"但主表物质数 = {n_main}，无法按顺序一一对应。"
    )
...
slope_all = (
    Internal_energy_Tb_all
    - Internal_energy_298_all
) / safe_denom

slope_df = pd.DataFrame({
    "Material_ID": material_ids,
    "Internal_energy_298_pred": Internal_energy_298_all,
    "Internal_energy_Tb_pred": Internal_energy_Tb_all,
    "Tb_pred": Tb_pred_all,
    "slope": slope_all
})
...
trans_df["Internal_energy_298_pred"] = np.repeat(
    slope_df["Internal_energy_298_pred"].values,
    rows_per_material
)

trans_df["Internal_energy_Tb_pred"] = np.repeat(
    slope_df["Internal_energy_Tb_pred"].values,
    rows_per_material
)
...
print("\nTransformed Internal Energy + slope RF 完整数据集预测偏差 1%，5%，10%分别为：")
...
all_result["Predicted_Internal_Energy"] = y_all_pred
...
feature_importance_file = "Transformed_Internal_Energy_with_slope_RF_feature_importance.xlsx"
...
# 16. 输出模型结构记录
# =========================
print("\n当前 Transformed Internal Energy + slope + RF 模型结构:")
print("Internal_energy_298_submodel: RandomForestRegressor(n_estimators=100, random_state=50, n_jobs=-1)")
print("Internal_energy_Tb_submodel: RandomForestRegressor(n_estimators=100, random_state=50, n_jobs=-1)")
print("Tb_submodel: HuberRegressor(max_iter=10000), input = PolynomialFeatures(Nk, degree=2)")
print("slope = (Internal_energy_Tb_pred - Internal_energy_298_pred) / (Tb_pred - 298.15)")
print("Final target: Internal energy")
print("Final model: RandomForestRegressor(n_estimators=100, random_state=50, n_jobs=-1)")
print("Final input features: transformed numeric features + slope + Internal_energy_298_pred + Internal_energy_Tb_pred + Tb_pred")
