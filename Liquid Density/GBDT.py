...
#         vols = pd.to_numeric(row[v_cols], errors="coerce").values
#
#         # 基团缺失则整行跳过
...
#             if not np.isfinite(T) or not np.isfinite(vol):
#                 continue
#
...
#
# output_file = "Liquid_density_GBR_train_test_split_by_material.xlsx"
...
v_cols = df.columns[41:51]           # 10个密度列
...
print("\nLiquid Density GBDT 完整数据集预测偏差 1%，5%，10%分别为：")
...
feature_importance_file = "Liquid_density_GBDT_feature_importance.xlsx"
...
# 17. 输出模型结构记录
# ============================================================
print("\n当前 Liquid Density GBDT 直接预测模型结构:")
