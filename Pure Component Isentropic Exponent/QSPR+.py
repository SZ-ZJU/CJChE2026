...
# # 0. 参数区
# # =========================
# main_file = "pure component isentropic exponent 207.xlsx"
# main_sheet = "Sheet1"
#
# normal_desc_file = "selected_25_descriptors_normal.xlsx"
# normal_target = "ASPEN isentropic exponent at normal Temperature(bar)"
#
# boiling_desc_file = "selected_25_descriptors_boiling.xlsx"
# boiling_target = "ASPEN isentropic exponent at boiling Temperature(bar)"
#
# transformed_file = "Transformed_volume_Dataset.xlsx"
# final_target_col = "isentropic exponent"
#
...
#     "isentropic_normal_pred": HVap_298_all,
#     "isentropic_boiling_pred": HVap_Tb_all,
#     "Tb_pred": Tb_pred_all,
#     "slope": slope_all
# })
...
#
# transformed_with_slope_path = "Transformed_isentropic_exponent_with_slope.xlsx"
...
# train_result["Predicted_isentropic_exponent"] = y_train_pred
...
# test_result["Predicted_isentropic_exponent"] = y_test_pred
...
#
# final_output = "prediction_vs_actual_isentropic_exponent_with_slope_train_test_split.xlsx"
...
isentropic_normal_all = rf_298.predict(X_298_full)
...
isentropic_boiling_all = rf_Tb.predict(X_Tb_full)
...
if len(isentropic_normal_all) != n_main:
    raise ValueError(
        f"{normal_desc_file} 预测行数 = {len(isentropic_normal_all)}，"
        f"但主表物质数 = {n_main}，无法按顺序一一对应。"
    )

if len(isentropic_boiling_all) != n_main:
    raise ValueError(
        f"{boiling_desc_file} 预测行数 = {len(isentropic_boiling_all)}，"
        f"但主表物质数 = {n_main}，无法按顺序一一对应。"
    )
...
slope_all = (
    isentropic_boiling_all
    - isentropic_normal_all
) / safe_denom

slope_df = pd.DataFrame({
    "Material_ID": material_ids,
    "isentropic_normal_pred": isentropic_normal_all,
    "isentropic_boiling_pred": isentropic_boiling_all,
    "Tb_pred": Tb_pred_all,
    "slope": slope_all
})
...
# 建议保留参考预测值，便于后续解释和检查
trans_df["isentropic_normal_pred"] = np.repeat(
    slope_df["isentropic_normal_pred"].values,
    rows_per_material
)

trans_df["isentropic_boiling_pred"] = np.repeat(
    slope_df["isentropic_boiling_pred"].values,
    rows_per_material
)
...
print("\nTransformed Isentropic Exponent + slope RF 完整数据集预测偏差 1%，5%，10%分别为：")
...
all_result["Predicted_isentropic_exponent"] = y_all_pred
...
feature_importance_file = "Transformed_Isentropic_Exponent_with_slope_RF_feature_importance.xlsx"
...
# 16. 输出模型结构记录
# =========================
print("\n当前 Transformed Isentropic Exponent + slope + RF 模型结构:")
print("Isentropic_normal_submodel: RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=-1)")
print("Isentropic_boiling_submodel: RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=-1)")
print("Tb_submodel: HuberRegressor(max_iter=10000), input = PolynomialFeatures(Nk, degree=2)")
print("slope = (isentropic_boiling_pred - isentropic_normal_pred) / (Tb_pred - 298.15)")
print("Final target: isentropic exponent")
print("Final model: RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=-1)")
print("Final input features: transformed numeric features + slope + isentropic_normal_pred + isentropic_boiling_pred + Tb_pred")
