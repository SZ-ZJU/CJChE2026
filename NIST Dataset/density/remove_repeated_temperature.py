...
# 1. 输入输出文件
# =========================================================
input_file = Path("thermoml_Density_Liquid_around_100kPa.xlsx")
input_sheet = "Liquid_around_100kPa"

output_file = Path("thermoml_Density_Liquid_around_100kPa_deduplicated_min3_Tgt30.xlsx")
...
for material_key, group in df_after_min_points.groupby("material_key", sort=False):
    ...
    row = {
        ...
        "T_max": T_max,
        "T_range": T_range,
        "density_min_g_per_cm3": np.nanmin(dens) if not np.all(np.isnan(dens)) else np.nan,
        "density_max_g_per_cm3": np.nanmax(dens) if not np.all(np.isnan(dens)) else np.nan,
    }
...
def build_material_summary(df_in):
    ...
    for material_key, group in df_in.groupby("material_key", sort=False):
        ...
        row = {
            ...
            "density_range_g_per_cm3": np.nanmax(dens) - np.nanmin(dens),
        }
...
if len(duplicate_summary) > 0:
    # 用原始数据取物质信息，避免有些物质最终被删除后无法 merge
    material_info_cols = ["material_key", "compound_name", "cas", "formula", "inchikey"]
...
if len(too_few_summary) > 0:
    ...
    for material_key, group in df_removed_too_few_points.groupby("material_key", sort=False):
        ...
        row["density_min_g_per_cm3"] = np.nanmin(dens) if not np.all(np.isnan(dens)) else np.nan
        row["density_max_g_per_cm3"] = np.nanmax(dens) if not np.all(np.isnan(dens)) else np.nan
...
with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
    df_final.to_excel(writer, sheet_name="Liquid_100kPa_Final", index=False)
