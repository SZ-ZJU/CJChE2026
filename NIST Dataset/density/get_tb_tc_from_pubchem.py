...
# 1. 输入输出文件
# =========================================================
input_file = Path("thermoml_Density_Liquid_100kPa_with_clean_SMILES.xlsx")
input_sheet = "Data_Clean_SMILES"

output_file = Path("thermoml_Density_Liquid_100kPa_clean_SMILES_with_Tb_Tc.xlsx")
...
def main():
    ...
    print("Data_Clean_SMILES 数据行数:", len(df_data))
    ...
    summary = pd.DataFrame([
        ...
        {"item": "Tb_Tc_ok_materials", "value": int((df_material_out["Tb_Tc_status"] == "ok").sum())},
        ...
        {"item": "materials_with_critical_T", "value": int(df_material_out["critical_T_K"].notna().sum())},
    ])

    # ---------- 保存 ----------
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        df_data_out.to_excel(writer, sheet_name="Data_with_Tb_Tc", index=False)
        df_material_out.to_excel(writer, sheet_name="Material_with_Tb_Tc", index=False)
