...
# 1. 文件路径
# =========================================================
# 清理后的 density Mordred 描述符文件
descriptor_file = Path("density_descriptors_mordred_2d_cleaned.csv")

# density 原始数据文件
density_excel_file = Path("dataset_density_selected_by_two_k_with_density_T_interpolation_8points.xlsx")

# 优先使用 Data_selected，因为它是一行一个温度点，包含原始 density 数值
density_sheet_name = "Data_selected"

# 输出文件
output_excel = Path("selected_descriptors_with_density_mean_target.xlsx")
output_txt = Path("selected_density_descriptors.txt")
...
# 5. 读取 density 原始数据
# =========================================================
df_density = pd.read_excel(density_excel_file, sheet_name=density_sheet_name)

print("Density Sheet 行数:", len(df_density))
print("Density Sheet 列数:", len(df_density.columns))

density_col = find_first_existing_col(
    df_density,
    density_col_candidates,
    required=True,
    col_type="density 目标列"
)

print("使用 density 目标列:", density_col)

df_density[density_col] = pd.to_numeric(
    df_density[density_col],
    errors="coerce"
)


# =========================================================
# 6. 按物质计算 density 平均目标值
# =========================================================
# 优先使用 material_key 分组；否则用 original_material_index；
# 如果都没有，则默认每 8 行一个物质。
if "material_key" in df_density.columns:
    group_col = "material_key"

    df_density[group_col] = df_density[group_col].apply(clean_key_value)

    grouped = df_density.groupby(group_col, sort=False)

    material_info_rows = []

    for material_idx, (key, sub) in enumerate(grouped):
        values = pd.to_numeric(sub[density_col], errors="coerce").values.astype(float)
        ...
        info = {
            ...
            "material_key": key,
            "target_mean_density": target_mean,
            "target_n_valid_points": len(valid_values),
            "target_min_density": target_min,
            "target_max_density": target_max,
        }
...
elif "original_material_index" in df_density.columns:
    material_info_rows = []
    ...
    if len(df_density) % n_points_per_material != 0:
        raise ValueError(
            f"{density_sheet_name} 行数 {len(df_density)} 不能被 "
            f"{n_points_per_material} 整除，且没有 material_key 或 "
            "original_material_index，无法确定每个物质。"
        )

    n_materials_density = len(df_density) // n_points_per_material
    print("按每 8 行一个物质处理，物质数:", n_materials_density)

    material_info_rows = []

    for material_idx in range(n_materials_density):
        start = material_idx * n_points_per_material
        end = start + n_points_per_material

        sub = df_density.iloc[start:end].copy()
...
print("Density 数据中的物质数:", len(df_material_info))


# =========================================================
# 7. 对齐描述符表和 density 目标表
# =========================================================
...
if desc_key_col is not None and info_key_col is not None:
    print(f"使用键对齐描述符和 density 目标: {desc_key_col} <-> {info_key_col}")
    ...
    if len(df_merged) == 0:
        raise ValueError(
            "使用键对齐后没有匹配到任何物质。"
            "请检查描述符表和 density 原始表中的 ID 是否一致。"
        )

    print("对齐后物质数:", len(df_merged))

    # 目标信息列：来自 df_material_info
    ...
    if len(df_desc_raw) != len(df_material_info):
        raise ValueError(
            "描述符表行数和 density 物质数不一致，且没有可用 ID 列对齐。\n"
            f"描述符表行数 = {len(df_desc_raw)}\n"
            f"density 物质数 = {len(df_material_info)}"
        )
...
# =========================================================
y_all = pd.to_numeric(
    df_material_info_valid["target_mean_density"],
    errors="coerce"
).values.astype(float)
...
if len(y) == 0:
    raise ValueError("没有有效 density 平均目标值，无法筛选描述符。")

# =========================================================
# 9. 构造数值描述符矩阵
# =========================================================
# 需要排除元信息列，只保留 Mordred 数值描述符
metadata_candidates = [
    "source_sdf",
    "mol_index_in_file",
    "global_mol_index",
...
df_output = pd.concat(
    [
        ...
        pd.DataFrame({"target_mean_density": y})
    ],
    axis=1
)
...
df_summary = pd.DataFrame([
    ...
    {"item": "density_excel_file", "value": str(density_excel_file)},
    {"item": "density_sheet_name", "value": density_sheet_name},
    {"item": "density_target_col", "value": density_col},
