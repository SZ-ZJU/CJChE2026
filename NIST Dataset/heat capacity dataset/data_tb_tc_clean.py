...
sheet_groups = "groups"      # 每个物质 1 行，顺序与 Sheet2 一致
...
def main():
    ...
    df_groups = pd.read_excel(input_file, sheet_name=sheet_groups)

    print("Sheet1 数据行数:", len(df_data))
    print("Sheet2 物质数:", len(df_material))
    print("groups 物质数:", len(df_groups))

    # ---------- 检查三个 sheet 是否按同一物质顺序排列 ----------
    if len(df_material) != len(df_groups):
        raise ValueError(
            f"Sheet2 物质数 {len(df_material)} 与 groups 行数 {len(df_groups)} 不一致。"
            "当前过滤逻辑要求 Sheet2 与 groups 按相同物质顺序一一对应。"
        )

    if len(df_data) != len(df_material) * n_points_per_material:
        raise ValueError(
            f"Sheet1 行数 {len(df_data)} 不等于 Sheet2 物质数 × {n_points_per_material}。"
            "请检查是否每个物质都是 8 行。"
        )
    ...
    # =========================================================
    # 额外生成：只保留有沸点 boiling_T_K 的物质
    # =========================================================
    has_boiling_mask = df_material_out["boiling_T_K"].notna().to_numpy()

    print("\n有沸点物质过滤：")
    print("原始物质数:", len(df_material_out))
    print("有沸点物质数:", int(has_boiling_mask.sum()))
    print("无沸点物质数:", int((~has_boiling_mask).sum()))

    # ---------- Sheet2：每个物质一行 ----------
    df_material_boiling_only = df_material_out.loc[has_boiling_mask].copy()

    # ---------- groups：每个物质一行，顺序与 Sheet2 一致 ----------
    df_groups_boiling_only = df_groups.loc[has_boiling_mask].copy()

    # ---------- Sheet1：每个物质 8 行，按顺序同步删除 ----------
    keep_data_indices = []

    for material_idx, keep in enumerate(has_boiling_mask):
        if keep:
            start = material_idx * n_points_per_material
            end = start + n_points_per_material
            keep_data_indices.extend(range(start, end))

    df_data_boiling_only = df_data_out.iloc[keep_data_indices].copy()

    print("Sheet1 原始行数:", len(df_data_out))
    print("Sheet1 有沸点过滤后行数:", len(df_data_boiling_only))
    print("Sheet2 原始物质数:", len(df_material_out))
    print("Sheet2 有沸点过滤后物质数:", len(df_material_boiling_only))
    print("groups 原始行数:", len(df_groups))
    print("groups 有沸点过滤后行数:", len(df_groups_boiling_only))

    # ---------- 没有沸点的物质 ----------
    no_boiling = df_material_out[df_material_out["boiling_T_K"].isna()].copy()
    ...
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        ...
        df_groups.to_excel(writer, sheet_name="groups_original", index=False)

        # 新增：只保留有沸点的物质
        df_data_boiling_only.to_excel(writer, sheet_name="Sheet1_with_boiling", index=False)
        df_material_boiling_only.to_excel(writer, sheet_name="Sheet2_with_boiling", index=False)
        df_groups_boiling_only.to_excel(writer, sheet_name="groups_with_boiling", index=False)
        ...
        no_boiling.to_excel(writer, sheet_name="No_Boiling_Point", index=False)
