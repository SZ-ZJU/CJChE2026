...
output_file = Path("Group_prediction_for_k1_k2_boiling_GBDT_with_predicted_slope.xlsx")
...
# 12. 使用预测结果计算新的 slope
# slope_pred =
# (Cp2_pred - Cp1_pred) / (T2_pred - T1_pred)
#
# 其中：
# T1_pred = k1 * boiling_T_K_pred
# T2_pred = k2 * boiling_T_K_pred
# =========================================================
idx_cp1 = target_cols.index("property_interp_at_k1Tb")
idx_cp2 = target_cols.index("property_interp_at_k2Tb")
idx_tb = target_cols.index("boiling_T_K")

cp1_exp = Y[:, idx_cp1]
cp2_exp = Y[:, idx_cp2]
tb_exp = Y[:, idx_tb]

cp1_pred = Y_pred[:, idx_cp1]
cp2_pred = Y_pred[:, idx_cp2]
tb_pred = Y_pred[:, idx_tb]


# ---------- 读取 k1, k2 ----------
def get_k_values(df, k_col, ref_T_col, tb_col):
    """
    优先读取 k_col。
    如果没有 k_col，则用 ref_T_col / tb_col 反推。
    """
    if k_col in df.columns:
        return pd.to_numeric(df[k_col], errors="coerce").values.astype(float)

    if ref_T_col in df.columns and tb_col in df.columns:
        ref_T = pd.to_numeric(df[ref_T_col], errors="coerce").values.astype(float)
        tb = pd.to_numeric(df[tb_col], errors="coerce").values.astype(float)
    ...
    raise ValueError(f"无法获得 {k_col}，也无法通过 {ref_T_col}/{tb_col} 反推。")


k1_values = get_k_values(
    df_model_valid,
    k_col="k1",
    ref_T_col="k1_times_boiling_T_K",
    tb_col="boiling_T_K",
)

k2_values = get_k_values(
    df_model_valid,
    k_col="k2",
    ref_T_col="k2_times_boiling_T_K",
    tb_col="boiling_T_K",
)


# ---------- 实验参考温度 ----------
# 优先使用原表中的 k1_times_boiling_T_K / k2_times_boiling_T_K
if "k1_times_boiling_T_K" in df_model_valid.columns:
    T1_exp = pd.to_numeric(
        df_model_valid["k1_times_boiling_T_K"],
        errors="coerce"
    ).values.astype(float)
else:
    T1_exp = k1_values * tb_exp

if "k2_times_boiling_T_K" in df_model_valid.columns:
    T2_exp = pd.to_numeric(
        df_model_valid["k2_times_boiling_T_K"],
        errors="coerce"
    ).values.astype(float)
else:
    T2_exp = k2_values * tb_exp


# ---------- 预测参考温度 ----------
T1_pred = k1_values * tb_pred
T2_pred = k2_values * tb_pred


# ---------- 实验 slope ----------
delta_cp_exp = cp2_exp - cp1_exp
delta_T_exp = T2_exp - T1_exp

slope_exp = np.where(
    np.abs(delta_T_exp) > 1e-12,
    delta_cp_exp / delta_T_exp,
    np.nan
)


# ---------- 预测 slope ----------
delta_cp_pred = cp2_pred - cp1_pred
delta_T_pred = T2_pred - T1_pred

slope_pred = np.where(
    np.abs(delta_T_pred) > 1e-12,
    delta_cp_pred / delta_T_pred,
    np.nan
)


# ---------- slope 误差 ----------
slope_error = slope_pred - slope_exp
slope_abs_error = np.abs(slope_error)

slope_relative_error_percent = np.where(
    np.abs(slope_exp) > 1e-12,
    np.abs(slope_error / slope_exp) * 100,
    np.nan
)

slope_valid_mask = (
    np.isfinite(slope_exp)
    & np.isfinite(slope_pred)
)
...
# 13. slope 评价指标
# =========================================================
def calc_slope_metrics(slope_true, slope_hat):
    mask = np.isfinite(slope_true) & np.isfinite(slope_hat)

    if mask.sum() == 0:
        return pd.DataFrame([{
            "target": "predicted_reference_slope",
            "n_points": 0,
            "R2_all": np.nan,
            "MSE": np.nan,
            "RMSE": np.nan,
    ...
    y_true = slope_true[mask]
    y_pred = slope_hat[mask]

    valid_rel = np.abs(y_true) > 1e-12

    if valid_rel.sum() > 0:
        rel = np.abs((y_pred[valid_rel] - y_true[valid_rel]) / y_true[valid_rel]) * 100
        ard = np.mean(rel)
        max_rel = np.max(rel)
    else:
        ard = np.nan
    ...
    return pd.DataFrame([{
        ...
        "MAE": mae,
        "ARD_percent": ard,
        "max_abs_error": np.max(np.abs(y_pred - y_true)),
        "max_relative_error_percent": max_rel,
    }])


df_slope_metrics = calc_slope_metrics(slope_exp, slope_pred)
...
# 加入 slope 相关列
df_prediction["T1_exp_K"] = T1_exp
df_prediction["T2_exp_K"] = T2_exp
df_prediction["delta_T_exp_K"] = delta_T_exp
df_prediction["delta_cp_exp"] = delta_cp_exp
df_prediction["reference_slope_exp"] = slope_exp

df_prediction["T1_pred_K"] = T1_pred
df_prediction["T2_pred_K"] = T2_pred
df_prediction["delta_T_pred_K"] = delta_T_pred
df_prediction["delta_cp_pred"] = delta_cp_pred
df_prediction["reference_slope_pred"] = slope_pred

df_prediction["reference_slope_error"] = slope_error
df_prediction["reference_slope_abs_error"] = slope_abs_error
df_prediction["reference_slope_relative_error_percent"] = slope_relative_error_percent
df_prediction["reference_slope_valid"] = slope_valid_mask
...
for col in [
    ...
    if col in df_model_valid.columns:
        df_slope[col] = df_model_valid[col].values

df_slope["k1"] = k1_values
df_slope["k2"] = k2_values

df_slope["cp1_exp"] = cp1_exp
df_slope["cp2_exp"] = cp2_exp
df_slope["boiling_T_K_exp"] = tb_exp
df_slope["T1_exp_K"] = T1_exp
df_slope["T2_exp_K"] = T2_exp
df_slope["delta_cp_exp"] = delta_cp_exp
df_slope["delta_T_exp_K"] = delta_T_exp
df_slope["reference_slope_exp"] = slope_exp

df_slope["cp1_pred"] = cp1_pred
df_slope["cp2_pred"] = cp2_pred
df_slope["boiling_T_K_pred"] = tb_pred
df_slope["T1_pred_K"] = T1_pred
df_slope["T2_pred_K"] = T2_pred
df_slope["delta_cp_pred"] = delta_cp_pred
df_slope["delta_T_pred_K"] = delta_T_pred
df_slope["reference_slope_pred"] = slope_pred

df_slope["reference_slope_error"] = slope_error
df_slope["reference_slope_abs_error"] = slope_abs_error
df_slope["reference_slope_relative_error_percent"] = slope_relative_error_percent
df_slope["reference_slope_valid"] = slope_valid_mask
...
df_model_params = pd.concat([
    pd.DataFrame([
        ...
        {"parameter": "slope_formula", "value": "(cp2_pred - cp1_pred) / (k2*Tb_pred - k1*Tb_pred)"},
    ]),
    df_model_params
...
with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
    ...
    df_slope.to_excel(writer, sheet_name="Predicted_Reference_Slope", index=False)
...
print("新增 sheet: Predicted_Reference_Slope")