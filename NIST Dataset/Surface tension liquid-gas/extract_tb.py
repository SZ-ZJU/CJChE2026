# -*- coding: utf-8 -*-
"""
Surface tension liquid-gas 剩余物质提取 Tb / boiling temperature 脚本

输入：
    thermoml_surface_tension_liquid_gas_Liquid_remove_dupT_materials_min3_Tgt20_RSQ_with_PubChem_SMILES.xlsx
    sheet: Data_Valid_SMILES_Only

输出：
    thermoml_surface_tension_liquid_gas_Liquid_remove_dupT_materials_min3_Tgt20_validSMILES_with_Tb.xlsx

功能：
    1. 读取已经清洗、计算线性度并补充 SMILES 的 Surface tension 数据
    2. 按 material_key 提取唯一物质
    3. 优先使用已有 pubchem_cid
    4. 否则按 inchikey -> pubchem_inchikey -> cas -> compound_name -> SMILES 查询 PubChem CID
    5. 从 PubChem PUG-View 的 Boiling Point 条目提取沸点
    6. 将 °C / °F / K 统一转换为 K
    7. 对同一物质多个 Tb 值取中位数
    8. 合并回原始数据表
"""
...
input_sheet = "Data_Valid_SMILES_Only"
...
def value_to_text(value):
    """
    将 PubChem PUG-View 中的 Value 字段尽量转成普通字符串。
    """
    if value is None:
        return ""
...
def plausible_boiling_temperature(value_K):
    ...
    有机物常见沸点大多在 150-700 K。
    这里放宽到 50-1000 K，避免误删特殊物质。
    """
...
def get_pubchem_cid_for_material(row):
    ...
    查询优先级：
        ...
        4. cas
        5. compound_name
        6. SMILES / final_smiles / pubchem_isomeric_smiles / pubchem_canonical_smiles / smiles
    """
    if "pubchem_cid" in row.index and is_valid_value(row["pubchem_cid"]):
        try:
            return int(float(row["pubchem_cid"])), "existing_pubchem_cid", row["pubchem_cid"]
        except Exception:
            pass

    for key_col in ["inchikey", "pubchem_inchikey"]:
        if key_col in row.index and is_valid_value(row[key_col]):
            cid, status = get_cid_by_namespace("inchikey", row[key_col])
            if cid is not None:
                return cid, key_col, row[key_col]
...
def extract_boiling_temperature_from_pubchem(cid):
    ...
    for heading in headings:
        ...
        for sec_row in section_rows:
            ...
            temp_values = parse_temperature_values_from_text(
                sec_row.get("value_text", "")
            )
...
def main():
    ...
    df_data_base = df_data.drop(columns=old_cols, errors="ignore")
    ...
    ok_materials = df_material_out[df_material_out["Tb_status"] == "ok"].copy()
    ...
    summary = pd.DataFrame([
        ...
        {
            "item": "Tb_ok_materials",
            "value": int((df_material_out["Tb_status"] == "ok").sum()),
        },
        {
            "item": "failed_or_not_found_materials",
            "value": len(failed),
        },
        {
            "item": "materials_with_boiling_T",
            "value": int(df_material_out["boiling_T_K"].notna().sum()),
        },
        {
            "item": "Tb_source",
            "value": "PubChem PUG-View Boiling Point",
        },
        {
            "item": "Tb_value_rule",
            "value": "median of parsed plausible boiling temperatures",
        },
        {
            "item": "cid_query_priority",
            "value": "existing_pubchem_cid -> inchikey -> pubchem_inchikey -> cas -> compound_name -> SMILES/final_smiles/pubchem_smiles",
        },
    ])
    ...
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        ...
        ok_materials.to_excel(writer, sheet_name="Tb_OK_Materials", index=False)
