# -*- coding: utf-8 -*-
"""
Surface tension liquid-gas Mordred 2D 描述符清理脚本

输入：
    surface_tension_descriptors_mordred_2d.csv

输出：
    surface_tension_descriptors_mordred_2d_cleaned.csv
    surface_tension_descriptors_mordred_2d_cleaned.xlsx

清理规则：
    1. 元信息列不参与描述符清理，但最终保留
    2. 描述符列中 inf / -inf 替换为 NaN
    3. 删除缺失比例 >= 30% 的描述符列
    4. 将描述符列统一转为数值型
    5. 删除转数值后全是 NaN 的列
    6. 删除零方差列
    7. 输出清理后的描述符表、元信息表、删除列报告和 summary
"""
...
input_file = Path("surface_tension_descriptors_mordred_2d.csv")

output_csv = Path("surface_tension_descriptors_mordred_2d_cleaned.csv")
output_excel = Path("surface_tension_descriptors_mordred_2d_cleaned.xlsx")
...
summary = pd.DataFrame([
    ...
    {"item": "output_csv", "value": str(output_csv)},
    {"item": "output_excel", "value": str(output_excel)},
    ...
    {"item": "descriptor_source", "value": "Mordred 2D descriptors"},
])
...
with pd.ExcelWriter(output_excel, engine="openpyxl") as writer:
    ...
    missing_ratio_report.to_excel(
        writer,
        sheet_name="missing_ratio_report",
        index=False,
    )

    variance_report.to_excel(
        writer,
        sheet_name="variance_report",
        index=False,
    )
...
print("\n删除统计:")
print("高缺失列:", len(drop_high_missing_cols))
print("非数值列:", len(drop_non_numeric_cols))
print("零方差列:", len(drop_zero_var_cols))