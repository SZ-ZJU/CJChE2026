...
# 1. 输入输出文件
# =========================================================
# 只读取按相态划分后的 vapor pressure 文件中的 Liquid sheet
input_file = Path("thermoml_vapor_pressure_by_phase.xlsx")
input_sheet = "Liquid"

output_file = Path("thermoml_vapor_pressure_Liquid_deduplicated_min3_Tgt30.xlsx")
...
# 是否删除非正蒸气压
# 因为后面建模 lnP 时，P <= 0 没有意义
remove_non_positive_vp = True
...
if lnp_col not in df.columns:
    print(f"警告：没有找到 {lnp_col}，后面会根据 {vp_col} 自动生成。")
...
print("\n删除 T 或 vapor pressure 为空的数据点数:", before_drop_na - len(df))

# 删除非正蒸气压
if remove_non_positive_vp:
...
for material_key, group in df_after_min_points.groupby("material_key", sort=False):
    ...
    vps = pd.to_numeric(group[vp_col], errors="coerce").values.astype(float)
    lnps = pd.to_numeric(group[lnp_col], errors="coerce").values.astype(float)
    ...
    row = {
        ...
        "T_max": T_max,
        "T_range": T_range,
        "VaporPressure_min_kPa": np.nanmin(vps) if not np.all(np.isnan(vps)) else np.nan,
        "VaporPressure_max_kPa": np.nanmax(vps) if not np.all(np.isnan(vps)) else np.nan,
        "VaporPressure_range_kPa": (
            np.nanmax(vps) - np.nanmin(vps)
            if not np.all(np.isnan(vps))
            else np.nan
        ),
        "lnP_min_kPa": np.nanmin(lnps) if not np.all(np.isnan(lnps)) else np.nan,
        "lnP_max_kPa": np.nanmax(lnps) if not np.all(np.isnan(lnps)) else np.nan,
        "lnP_range_kPa": (
            np.nanmax(lnps) - np.nanmin(lnps)
            if not np.all(np.isnan(lnps))
            else np.nan
        ),
    }
...
def build_material_summary(df_in):
    ...
    for material_key, group in df_in.groupby("material_key", sort=False):
        ...
        row = {
            ...
            "VaporPressure_min_kPa": np.nanmin(vps),
            "VaporPressure_max_kPa": np.nanmax(vps),
            "VaporPressure_range_kPa": np.nanmax(vps) - np.nanmin(vps),
            "lnP_min_kPa": np.nanmin(lnps),
            "lnP_max_kPa": np.nanmax(lnps),
            "lnP_range_kPa": np.nanmax(lnps) - np.nanmin(lnps),
        }
...
if len(too_few_summary) > 0:
    ...
    for material_key, group in df_removed_too_few_points.groupby("material_key", sort=False):
        ...
        row["VaporPressure_min_kPa"] = np.nanmin(vps) if not np.all(np.isnan(vps)) else np.nan
        row["VaporPressure_max_kPa"] = np.nanmax(vps) if not np.all(np.isnan(vps)) else np.nan
        row["lnP_min_kPa"] = np.nanmin(lnps) if not np.all(np.isnan(lnps)) else np.nan
        row["lnP_max_kPa"] = np.nanmax(lnps) if not np.all(np.isnan(lnps)) else np.nan
...
with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
    df_final.to_excel(writer, sheet_name="VP_Liquid_Final", index=False)
    ...
    run_info = pd.DataFrame([
        ...
        {"item": "vapor_pressure_col", "value": vp_col},
        ...
        {"item": "remove_non_positive_vp", "value": remove_non_positive_vp},
...
print("\n保存完成:", output_file)
print("最终主表 sheet 名: VP_Liquid_Final")