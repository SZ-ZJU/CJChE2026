...
# 1. 输入输出文件
# =========================================================
input_file = Path("thermoml_vapor_pressure_Liquid_with_clean_SMILES.xlsx")
input_sheet = "Data_Clean_SMILES"

output_file = Path("thermoml_vapor_pressure_Liquid_with_clean_SMILES_no_charged_N.xlsx")
...
for material_key, group in df.groupby("material_key", sort=False):
    ...
    for s in unique_smiles_values:
        frag = get_charged_nitrogen_fragments(s)
        if frag:
            charged_n_fragments.append(frag)

    charged_n_fragments = list(dict.fromkeys(charged_n_fragments))

    if len(unique_smiles_values) == 0:
        status = "empty_smiles"
        representative_smiles = None

    elif any(has_charged_nitrogen(s) for s in unique_smiles_values):
    ...
    row = {
        ...
        "charged_nitrogen_fragments": "; ".join(charged_n_fragments),
    }
