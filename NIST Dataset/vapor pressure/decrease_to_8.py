...
# 1. 输入输出文件
# =========================================================
input_file = Path("dataset_selected_by_two_k_with_lnP_invT_interpolation.xlsx")
output_file = Path("dataset_selected_by_two_k_with_lnP_invT_interpolation_8points.xlsx")
...
def select_evenly_with_max_temperature_range(group, temp_col, max_points=8):
    ...
    # 这里用 linspace(0, n-1, 8)，天然包含首尾点
    # round 后可能出现重复索引，所以需要去重。
    ...
    # 如果因为补首尾导致超过 8 个，则从中间点中均匀保留 6 个
    if len(selected_set) > max_points:
        middle_candidates = sorted([idx for idx in selected_set if idx not in [0, n - 1]])

        if max_points > 2:
            if len(middle_candidates) <= max_points - 2:
                middle_selected = middle_candidates
            else:
                middle_pos = np.linspace(0, len(middle_candidates) - 1, max_points - 2)
                middle_selected = [
...
def build_point_summary(df_data_before, df_data_after):
    ...
    for material_key in all_keys:
        ...
        for col in ["compound_name", "cas", "formula", "SMILES", "smiles", "boiling_T_K"]:
...
if material_sheet in sheet_tables:
    ...
    df_material = df_material.drop(
        columns=[c for c in summary_cols if c != material_key_col and c in df_material.columns],
        errors="ignore"
    )
...
# 如果 Covered_By_Both_k 或 Interpolated_k1_k2 存在，也可以不动
# 它们是物质级信息，不需要按温度点抽样。
...
run_info = pd.DataFrame([
    ...
    {"item": "selection_rule", "value": "if n<=8 keep all; if n>8 keep Tmin/Tmax and choose middle points evenly by sorted temperature"},
    ...
    {"item": "materials_with_more_than_8_before", "value": int((df_data.groupby(material_key_col).size() > 8).sum())},
    {"item": "materials_with_more_than_8_after", "value": int((df_data_8points.groupby(material_key_col).size() > 8).sum())},
])
