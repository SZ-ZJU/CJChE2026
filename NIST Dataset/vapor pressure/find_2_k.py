...
output_file = Path("dataset_selected_by_two_k_with_lnP_invT_interpolation.xlsx")
...
# lnP 列，优先自动识别；如果没有，则用 log(pressure_col) 自动生成
lnp_col = None
lnp_col_candidates = [
    "lnP_kPa",
...
def auto_find_column(df, candidates, col_type):
    ...
    raise ValueError(
        f"没有找到 {col_type} 列。候选列名为：{candidates}\n"
...
def interpolate_lnP_vs_invT_one_material(group, target_T, temp_col, lnp_col):
    """
    对单个物质，在 lnP vs 1/T 空间做线性插值。

    不是直接对 P vs T 插值，而是：
        x = 1 / T
        y = ln(P)

    然后在 target_T 对应的 1/target_T 上插值得到 lnP。
    """
    g = group[[temp_col, lnp_col]].copy()

    g[temp_col] = pd.to_numeric(g[temp_col], errors="coerce")
    g[lnp_col] = pd.to_numeric(g[lnp_col], errors="coerce")

    g = g.dropna(subset=[temp_col, lnp_col])
    g = g[np.isfinite(g[temp_col]) & np.isfinite(g[lnp_col])]

    if len(g) < 2:
        return np.nan, np.nan, "failed_less_than_2_points"

    # 同一个温度如果有重复值，对 lnP 取平均
    g = (
        g
        .groupby(temp_col, as_index=False)[lnp_col]
        .mean()
    ...
    lnP_values = g[lnp_col].values.astype(float)
    ...
    try:
        lnP_interp = float(np.interp(target_x, x_sorted, y_sorted))
        P_interp = float(np.exp(lnP_interp))

        if not np.isfinite(lnP_interp) or not np.isfinite(P_interp):
            return np.nan, np.nan, "failed_nonfinite_result"

        return lnP_interp, P_interp, "ok_lnP_invT_linear_interpolation"
...
# group sheet 如果没有 material_key，也按 Material 顺序补上
...
if pressure_col is None:
    pressure_col = auto_find_column(df_data, pressure_col_candidates, "蒸汽压")

if lnp_col is None:
    existing_lnp = [col for col in lnp_col_candidates if col in df_data.columns]

    if len(existing_lnp) > 0:
        lnp_col = existing_lnp[0]
    else:
        lnp_col = "lnP_kPa"
        df_data[pressure_col] = pd.to_numeric(df_data[pressure_col], errors="coerce")
        df_data[lnp_col] = np.log(df_data[pressure_col])

print("使用的蒸汽压列:", pressure_col)
print("使用的 lnP 列:", lnp_col)
...
df_data[pressure_col] = pd.to_numeric(df_data[pressure_col], errors="coerce")
df_data[lnp_col] = pd.to_numeric(df_data[lnp_col], errors="coerce")
...
for idx, mat_row in df_material.iterrows():
    ...
    if len(group) == 0:
        ...
        g_valid = group.dropna(subset=[temp_col, pressure_col, lnp_col]).copy()
        g_valid = g_valid[
            np.isfinite(g_valid[temp_col])
            & np.isfinite(g_valid[pressure_col])
            & np.isfinite(g_valid[lnp_col])
            & (g_valid[pressure_col] > 0)
        ]
        ...
        if n_points > 0:
            ...
            T_range = T_max - T_min
            P_min = float(g_valid[pressure_col].min())
            P_max = float(g_valid[pressure_col].max())
            lnP_min = float(g_valid[lnp_col].min())
            lnP_max = float(g_valid[lnp_col].max())
        else:
            T_min = np.nan
            T_max = np.nan
            T_range = np.nan
...
# =========================================================
# 16. 在 k1*Tb 和 k2*Tb 处计算 lnP / P 插值理论值
# =========================================================
interp_rows = []
...
for _, row in df_selected_info.sort_values("original_material_index").iterrows():
    ...
    lnP_k1, P_k1, status_k1 = interpolate_lnP_vs_invT_one_material(
        group,
        T_k1Tb,
        temp_col=temp_col,
        lnp_col=lnp_col,
    ...
    lnP_k2, P_k2, status_k2 = interpolate_lnP_vs_invT_one_material(
        group,
        T_k2Tb,
        temp_col=temp_col,
        lnp_col=lnp_col,
    ...
    out = {
        ...
        "lnP_kPa_interp_at_k1Tb": lnP_k1,
        "VaporPressure_kPa_interp_at_k1Tb": P_k1,
        "interp_status_k1": status_k1,

        "k2": best_k2,
        "k2_times_boiling_T_K": T_k2Tb,
        "lnP_kPa_interp_at_k2Tb": lnP_k2,
        "VaporPressure_kPa_interp_at_k2Tb": P_k2,
...
print("\n========== 插值结果 ==========")
print("k1 插值成功物质数:", int((df_interpolated["interp_status_k1"] == "ok_lnP_invT_linear_interpolation").sum()))
print("k2 插值成功物质数:", int((df_interpolated["interp_status_k2"] == "ok_lnP_invT_linear_interpolation").sum()))
...
interp_cols_for_model = [
    ...
    "VaporPressure_kPa_interp_at_k1Tb",
    "k2",
    "k2_times_boiling_T_K",
    "lnP_kPa_interp_at_k2Tb",
    "VaporPressure_kPa_interp_at_k2Tb",
    "boiling_T_K",
    "T_min",
    "T_max",
    "T_range",
...
summary = pd.DataFrame([
...

    {
        ...
        "value": int((df_interpolated["interp_status_k1"] == "ok_lnP_invT_linear_interpolation").sum())
    },
    {
        "item": "k2_interpolation_success_count",
        "value": int((df_interpolated["interp_status_k2"] == "ok_lnP_invT_linear_interpolation").sum())
    },
])
...
print("\n保存完成:", output_file)
print("主要输出 sheet:")
print("1. Final_Model_Table：基团特征 + k1/k2 两个参考点理论蒸汽压")
