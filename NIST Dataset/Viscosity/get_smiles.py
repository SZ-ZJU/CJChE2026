...
output_file = Path("thermoml_viscosity_Liquid_with_PubChem_SMILES.xlsx")
...
material_smiles_cols = [c for c in material_smiles_cols if c in df_out.columns]

material_smiles_map = (
    df_out[material_smiles_cols]
    .drop_duplicates(subset=["material_key"])
    .reset_index(drop=True)
)
...
summary = pd.DataFrame([
    ...
    {"item": "final_smiles_priority", "value": "PubChem IsomericSMILES -> CanonicalSMILES -> ConnectivitySMILES -> original smiles"},
