...
# 1. 输入输出文件
# =========================================================
# 上一步 SDF -> Mordred 转换并回填后的 98 行描述符表
input_file = Path("viscosity_descriptors_mordred_2d.csv")

output_csv = Path("viscosity_descriptors_mordred_2d_cleaned.csv")
output_excel = Path("viscosity_descriptors_mordred_2d_cleaned.xlsx")
...
if not input_file.exists():
    raise FileNotFoundError(
        ...
        "请先运行 SDF -> Mordred 描述符转换代码，生成 viscosity_descriptors_mordred_2d.csv。"
    )
...
if len(df_clean) != len(df):
    print(
        f"警告：清理前后行数不一致。清理前 {len(df)} 行，清理后 {len(df_clean)} 行。"
    )
else:
    print(f"行数保持不变: {len(df_clean)} 行")
...
summary = pd.DataFrame([
    ...
    {"item": "row_count_preserved", "value": len(df_clean) == len(df)},
])
