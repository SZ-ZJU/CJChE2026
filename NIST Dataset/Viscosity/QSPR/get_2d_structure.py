...
# 如果最终只能从 Data_selected / Sheet1 这种长表里抽物质，
# 会优先按 material_key / original_material_index 等列分组；
# 只有完全没有物质 ID 时，才尝试每 8 行一个物质。
n_points_per_material = 8

# 输出文件
mapping_output = Path("viscosity_pubchem_cid_mapping_results.xlsx")

# SDF 输出
sdf_prefix = "viscosity_pubchem_2d_batch"
combined_sdf_file = Path("viscosity_pubchem_2d_all.sdf")
...
for s in preferred_sheet_candidates:
...
if sheet_name is None:
    ...
    print(f"没有找到候选 sheet，默认使用第一个 sheet: {sheet_name}")
...
# 4. 整理为每个物质一行
# =========================================================
def pick_material_table(df, sheet_name):
    """
    把原始表整理成一行一个物质。
    如果已经是一行一个物质，直接返回；
    如果是一行一个温度点，则按物质 ID 分组取第一行。
    """

    if "Material" in sheet_name or "material" in sheet_name:
        ...
        return material_df, material_index_col
    ...
    if len(df) % n_points_per_material != 0:
        raise ValueError(
            ...
            "且没有 original_material_index / material_key / pubchem_cid / inchikey / cas / compound_name，"
            "无法确定每个物质。请检查表格中是否有物质 ID 列。"
        )
...
material_df, material_index_col = pick_material_table(df, sheet_name)
