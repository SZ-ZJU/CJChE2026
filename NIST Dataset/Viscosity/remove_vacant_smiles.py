...
# 1. 输入输出文件
# =========================================================
input_file = Path("thermoml_viscosity_Liquid_with_PubChem_SMILES.xlsx")
input_sheet = "Data_With_SMILES"

output_file = Path("thermoml_viscosity_Liquid_with_clean_SMILES.xlsx")
...
# 是否额外删除带电氮。
# 如果你只想执行原始规则：空 SMILES + 含 "."，就保持 False。
# 如果你还想删除 [N+]、[N-]、[n+]、[n-]，改成 True。
...
def check_smiles_status(smiles):
    ...
    # 含 "." 的通常是盐、混合物、离子对、多组分结构
    if "." in s:
        return "smiles_contains_dot"

    if remove_charged_nitrogen and has_charged_nitrogen(s):
        return "charged_nitrogen_smiles"
...
for material_key, group in df.groupby("material_key", sort=False):
    ...
    if len(unique_smiles_values) == 0:
        ...
        statuses = [check_smiles_status(s) for s in unique_smiles_values]

        if "smiles_contains_dot" in statuses:
            status = "smiles_contains_dot"
        elif "charged_nitrogen_smiles" in statuses:
            status = "charged_nitrogen_smiles"
        elif all(st == "keep" for st in statuses):
            status = "keep"
        else:
            status = statuses[0]
...
summary = pd.DataFrame([
    ...
    {
        "item": "removed_charged_nitrogen_materials",
        "value": int((df_removed_materials["smiles_status"] == "charged_nitrogen_smiles").sum())
    },
])
...
print("\n保存完成:", output_file)
print("最终可用于 viscosity 建模的 sheet: Data_Clean_SMILES")