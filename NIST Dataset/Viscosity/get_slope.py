...
output_file = Path("HistGB_submodels_predict_ref_lnVisc_Tb_and_slope.xlsx")
...
# 三个子模型的目标列
target_lnvisc_k1_col = "lnViscosity_Pa_s_interp_at_k1Tb"
target_lnvisc_k2_col = "lnViscosity_Pa_s_interp_at_k2Tb"
target_Tb_col = "boiling_T_K"

# 如果你的列名略有不同，可以在这里加候选列自动识别
target_lnvisc_k1_candidates = [
    "lnViscosity_Pa_s_interp_at_k1Tb",
    "lnViscosity_interp_at_k1Tb",
    "ln_viscosity_interp_at_k1Tb",
    "ln_eta_interp_at_k1Tb",
]

target_lnvisc_k2_candidates = [
    "lnViscosity_Pa_s_interp_at_k2Tb",
    "lnViscosity_interp_at_k2Tb",
    "ln_viscosity_interp_at_k2Tb",
    "ln_eta_interp_at_k2Tb",
]
...
# 7. 自动识别目标列和参考温度列
# =========================================================
target_lnvisc_k1_col = find_first_existing_col(
    df_ref,
    target_lnvisc_k1_candidates,
    "k1 参考点 ln(viscosity)"
)

target_lnvisc_k2_col = find_first_existing_col(
    df_ref,
    target_lnvisc_k2_candidates,
    "k2 参考点 ln(viscosity)"
)
...
print("\n使用 k1 ln(viscosity) 目标列:", target_lnvisc_k1_col)
print("使用 k2 ln(viscosity) 目标列:", target_lnvisc_k2_col)
print("使用 Tb 目标列:", target_Tb_col)
print("使用 k1 参考温度列:", T_k1_col)
print("使用 k2 参考温度列:", T_k2_col)

required_ref_cols = [
    target_lnvisc_k1_col,
    target_lnvisc_k2_col,
    target_Tb_col,
    T_k1_col,
    T_k2_col,
]
...
df_submodel_clean = df_submodel_clean[
    np.isfinite(df_submodel_clean[target_lnvisc_k1_col])
    & np.isfinite(df_submodel_clean[target_lnvisc_k2_col])
...
y_lnvisc_k1 = df_submodel_clean[target_lnvisc_k1_col].values
y_lnvisc_k2 = df_submodel_clean[target_lnvisc_k2_col].values
y_Tb = df_submodel_clean[target_Tb_col].values

model_lnvisc_k1, pred_lnvisc_k1, metrics_lnvisc_k1 = train_hgb_submodel(
    X,
    y_lnvisc_k1,
    "lnViscosity_interp_at_k1Tb"
)

model_lnvisc_k2, pred_lnvisc_k2, metrics_lnvisc_k2 = train_hgb_submodel(
    X,
    y_lnvisc_k2,
    "lnViscosity_interp_at_k2Tb"
)
...
# 三个子模型预测值
df_slope["lnVisc_k1_pred_by_group_model"] = pred_lnvisc_k1
df_slope["lnVisc_k2_pred_by_group_model"] = pred_lnvisc_k2
...
df_slope["delta_lnVisc_pred"] = (
    df_slope["lnVisc_k2_pred_by_group_model"]
    - df_slope["lnVisc_k1_pred_by_group_model"]
)
...
df_slope["slope_pred_lnVisc_over_invT"] = (
    df_slope["delta_lnVisc_pred"] / df_slope["delta_invT_pred"]
)

df_slope.loc[
    ~np.isfinite(df_slope["slope_pred_lnVisc_over_invT"]),
    "slope_pred_lnVisc_over_invT"
] = np.nan
...
df_slope["delta_lnVisc_true_ref"] = (
    df_slope[target_lnvisc_k2_col]
    - df_slope[target_lnvisc_k1_col]
)
...
df_slope["slope_true_ref_lnVisc_over_invT"] = (
    df_slope["delta_lnVisc_true_ref"] / df_slope["delta_invT_true_ref"]
)

df_slope.loc[
    ~np.isfinite(df_slope["slope_true_ref_lnVisc_over_invT"]),
    "slope_true_ref_lnVisc_over_invT"
] = np.nan

# 粘度 Arrhenius 型关系：
# ln(eta) ≈ A + E_eta/R * (1/T)
# 所以 slope ≈ E_eta/R
R_J_per_mol_K = 8.314462618

df_slope["E_eta_est_pred_kJ_per_mol"] = (
    df_slope["slope_pred_lnVisc_over_invT"] * R_J_per_mol_K / 1000.0
)

df_slope["E_eta_est_true_ref_kJ_per_mol"] = (
    df_slope["slope_true_ref_lnVisc_over_invT"] * R_J_per_mol_K / 1000.0
)

# 顺便还原两个预测参考点的 viscosity
df_slope["Viscosity_k1_pred_by_group_model_Pa_s"] = np.exp(
    np.clip(df_slope["lnVisc_k1_pred_by_group_model"], -700, 700)
)

df_slope["Viscosity_k2_pred_by_group_model_Pa_s"] = np.exp(
    np.clip(df_slope["lnVisc_k2_pred_by_group_model"], -700, 700)
)
...
# =========================================================
print("\n" + "=" * 80)
print("三个 HistGradientBoosting 粘度子模型训练集内评价指标")
print("=" * 80)
print(df_metrics.to_string(index=False))

print("\n" + "=" * 80)
print("预测 slope 统计：slope_pred_lnVisc_over_invT")
print("=" * 80)
print(df_slope["slope_pred_lnVisc_over_invT"].describe())

print("\n" + "=" * 80)
print("由预测 slope 估算的 E_eta 统计，单位 kJ/mol")
print("=" * 80)
print(df_slope["E_eta_est_pred_kJ_per_mol"].describe())

print("\n" + "=" * 80)
print("真实参考点 slope 统计：slope_true_ref_lnVisc_over_invT")
print("=" * 80)
print(df_slope["slope_true_ref_lnVisc_over_invT"].describe())

print("\n" + "=" * 80)
print("由真实参考点 slope 估算的 E_eta 统计，单位 kJ/mol")
print("=" * 80)
print(df_slope["E_eta_est_true_ref_kJ_per_mol"].describe())
...
df_run_info = pd.DataFrame([
    ...
    {"item": "submodel_1_target", "value": target_lnvisc_k1_col},
    {"item": "submodel_2_target", "value": target_lnvisc_k2_col},
    {"item": "submodel_3_target", "value": target_Tb_col},

    {"item": "k1", "value": k1},
    {"item": "k2", "value": k2},
    {"item": "slope_formula", "value": "(lnVisc_k2_pred - lnVisc_k1_pred) / (1/(k2*Tb_pred) - 1/(k1*Tb_pred))"},
    {"item": "slope_feature_for_main_model", "value": "slope_pred_lnVisc_over_invT"},
    {"item": "physical_relation", "value": "ln(eta) ≈ A + E_eta/R * (1/T), so slope ≈ E_eta/R"},
...
print("后续主模型建议使用的 slope 特征列: slope_pred_lnVisc_over_invT")