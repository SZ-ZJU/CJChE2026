...
# v_cols = df.columns[41:51]      # 第42~51列：目标变量（火用）
#
# # ==== 2. 数据预处理 ====
...
#
# # ==== 13. 保存最终结果 ====
# out_path = "exergy_actual_vs_pred_with_residual_correction_train_test_split.xlsx"
#
...
v_cols = df.columns[41:51]      # 第42~51列：目标变量，火用 Exergy
...
# ============================================================
# 3. Exergy_Tb 子模型：boiling exergy，全数据训练
# ============================================================
df_Tb = pd.read_excel("selected_25_descriptors_boiling.xlsx")

tb_target_col = "ASPEN Exergy at BoilingTemperature(j/mol)"
...
HVap_Tb_all = np.full(len(df), np.nan, dtype=float)

predict_Tb_mask = X_Tb.notna().all(axis=1)

HVap_Tb_all[predict_Tb_mask] = rf_Tb.predict(
    X_Tb.loc[predict_Tb_mask]
)

if len(HVap_Tb_all) != len(df):
    raise ValueError(
        f"selected_25_descriptors_boiling.xlsx 预测行数 = {len(HVap_Tb_all)}，"
        f"与主表物质数 = {len(df)} 不一致。"
    )
...
for i in range(len(train_df)):
    for tcol, vcol in zip(temp_cols, v_cols):
        ...
        if (
            ...
            or not np.isfinite(HVap_Tb_train[i])
...
def build_baseline_predictions(df_part, G_part, Tb_pred_part, HVap_Tb_part):
    ...
    for i in range(len(df_part)):
        if (
            ...
            or not np.isfinite(HVap_Tb_part[i])
...
print("\nFinal_model 完整数据集 Exergy 预测偏差 1%，5%，10%分别为：")
...
# 15. 输出模型结构记录
# ============================================================
print("\n当前 Exergy baseline + GBDT residual 模型结构:")
print("Exergy_Tb_submodel: RandomForestRegressor(random_state=42), input = selected_25_descriptors_boiling.xlsx")
print("Tb_submodel: HuberRegressor(max_iter=10000), input = StandardScaler(PolynomialFeatures(Nk, degree=2))")
print("Baseline: Exergy_baseline = Exergy_Tb_pred + (T - Tb_pred) * sum(Ak * Nk)")
...
print("Residual target: Exergy_actual - Exergy_baseline")
print("Residual features: Nk + T + (T-Tb) + T/Tb + ln(T) + Exergy_baseline + Tb_pred + Exergy_Tb_pred")
print("Final prediction: Exergy_final = Exergy_baseline + residual_pred")
